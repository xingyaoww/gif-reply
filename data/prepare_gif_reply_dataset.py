...
parser.add_argument("input_filepath", help="filepath of the downloaded gif-reply dataset.")
parser.add_argument("output_filepath", help="filepath of the processed gif-reply dataset.")
parser.add_argument("gif_metadata_filepath", help="filepath of the downloaded gif metadata.")
parser.add_argument("twitter_credential_file", help="filepath for twitter credentials.")
parser.add_argument("tweets_output_file", help="filepath for intermediate tweets json.")
args = parser.parse_args()

def crawl_tweets_to_file(all_tweet_ids: List[str]):
    logging.info(f"Start crawling parent tweets to {args.tweets_output_file}.")
    outfile = args.tweets_output_file
    ...
    logging.info("Saw %d new tweet IDs; already crawled %d" %
          (len(tweet_ids), len(already_crawled)))

    with open(args.twitter_credential_file) as your_key_file:
...
def get_tweet_id_to_parent_text_mapping(all_tweet_ids: List[str]) -> Dict[str, str]:
    logging.info("Creating Tweet ID to parent text mapping.")
    all_tweet_ids = set(all_tweet_ids)
    mapping = {}
    with open(args.tweets_output_file) as f:
        for line in f:
            tweet = json.loads(line)
...
dataset["parent_text"] = dataset["parent_id"].apply(lambda x: tweet_id_to_parent_text.get(x, "")).apply(normalizeTweet)
logging.info("Parent text processed for each parent tweet.")

# Match `tags` for each `child_gif_id`
gif_id_to_tags = dict(pd.read_csv(args.gif_metadata_filepath)[["child_gif_id", "tags"]].to_numpy())
dataset["tags"] = dataset["child_gif_id"].apply(lambda x: gif_id_to_tags.get(x))
