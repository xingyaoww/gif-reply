...
from pytorch_transformers.modeling_bert import BertAttention
...
from utils.cbs import ConstrainedBeamSearch
from utils.cbs import select_best_beam_with_constraints
        ...
        if hasattr(config, 'classifier'):
            ...
            if config.classifier == 'linear':
                ...
                self.classifier = nn.Sequential(
                    ...
                    nn.Linear(
                        config.hidden_size *
                        config.cls_hidden_scale, self.config.num_labels,
                    ),
                )
        else:
            self.classifier = nn.Linear(
...
class ImageBertForMultipleChoice(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if hasattr(config, 'classifier'):
            ...
            if config.classifier == 'linear':
                self.classifier = nn.Linear(
                    config.num_choice*config.hidden_size, self.config.num_labels,
                )
            elif config.classifier == 'mlp':
                self.classifier = nn.Sequential(
                    nn.Linear(
    ...
    def forward(
        ...
        flat_img_feats = img_feats.view(
            -1, img_feats.size(-2),
            img_feats.size(-1),
        ) if img_feats is not None else None

        if isinstance(self.bert, BertImgModel):
            outputs = self.bert(
                flat_input_ids, position_ids=flat_position_ids, token_type_ids=flat_token_type_ids,
        ...
        reshaped_pool_output = pooled_output.view(
            -1, self.config.num_choice*(pooled_output.shape[1]),
        )
        ...
        if labels is not None:
            if self.loss_type == 'bce':
                loss = instance_bce_with_logits(
                    logits, labels.view(-1, self.config.num_labels),
                )
            else:
                loss_fct = CrossEntropyLoss()
...
class BertForImageCaptioning(CaptionPreTrainedModel):
    ...
    def encode_forward(
        self, input_ids, img_feats, attention_mask, masked_pos, masked_ids=None,
    ...
    def prepare_inputs_for_generation(self, curr_ids, past=None):
        ...
        def _slice(t, start, end):
            if t is None:
                return t
            assert t.shape == (
                batch_size, self.max_seq_len + self.od_labels_len,
            )
            return t[:, start: end]
        ...
        if past is None:
            ...
            def _remove_rows_cols(t, row_start, row_end, col_start, col_end):
                ...
                res = torch.cat(
                    [
                        torch.cat([t00, t01], dim=2), torch.cat(
                            [t10, t11],
                            dim=2,
                        ),
                    ], dim=1,
            ...
            masked_pos = _remove_elements(
                self.full_masked_pos, seq_start, seq_end,
            )
            token_type_ids = _remove_elements(
                self.full_token_type_ids, seq_start, seq_end,
            )
            position_ids = _remove_elements(
                self.full_position_ids, seq_start, seq_end,
            )
            ...
            token_type_ids = _slice(
                self.full_token_type_ids, start_pos, end_pos,
            )
            ...
            if self.prev_encoded_layers is None:
                ...
                assert past[0].shape[1] == 2 + \
                    self.od_labels_len + self.img_seq_len
        ...
        return {
            'input_ids': input_ids, 'img_feats': img_feats,
    ...
    def generate(
        self, img_feats, attention_mask, masked_pos, token_type_ids=None,
        ...
        self.od_labels_start_posid = max(
            od_labels_start_posid, self.max_seq_len,
        )
        if self.add_od_labels:
            ...
            self.od_label_ids = self._expand_for_beams(
                od_label_ids, num_beams,
                num_fsm_states,
            )
            input_ids = None
        else:
        ...
        if position_ids is None:
            ...
            position_ids = position_ids.unsqueeze(
                0,
            ).expand([batch_size, posids_len])

        cur_len = input_ids.shape[1]
        assert num_return_sequences == 1, 'not supported num_return_sequences != 1'
        effective_batch_size = batch_size

        self.img_feats = self._expand_for_beams(
            img_feats, num_beams, num_fsm_states,
        )
        self.full_attention_mask = self._expand_for_beams(
            attention_mask, num_beams, num_fsm_states,
        )
        self.full_masked_pos = self._expand_for_beams(
            masked_pos, num_beams, num_fsm_states,
        )
        self.full_token_type_ids = self._expand_for_beams(
            token_type_ids, num_beams, num_fsm_states,
        )
        self.full_position_ids = self._expand_for_beams(
            position_ids, num_beams, num_fsm_states,
        )
        self.full_head_mask = self._expand_for_beams(
            head_mask, num_beams, num_fsm_states,
        )
