...
class CaptionPreTrainedModel(PreTrainedModel):
    ...
    def _do_output_past(self, outputs):
        has_output_past = hasattr(
            self.config, 'output_past',
        ) and self.config.output_past
    ...
    def _generate_beam_search(
        ...
        logprobs = torch.zeros(
            batch_size, num_keep_best,
            dtype=torch.float,
        ).fill_(-1e5).to(input_ids.device)
        all_best = []
