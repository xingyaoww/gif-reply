...
class CaptionBertEncoder(BertEncoder):
    ...
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList([CaptionBertLayer(config)
    ...
    def __init__(self, config):
        ...
        if hasattr(config, 'classifier'):
            ...
            self.classifier = nn.Linear(
                config.hidden_size, self.config.num_labels)  # original
        self.init_weights()
...
class ImageBertForMultipleChoice(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if hasattr(config, 'classifier'):
            ...
            self.classifier = nn.Linear(
                config.num_choice*config.hidden_size, self.config.num_labels)  # original
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
        ...
        flat_img_feats = img_feats.view(-1, img_feats.size(-2),
                                        img_feats.size(-1)) if img_feats is not None else None
    ...
    def generate(self, img_feats, attention_mask, masked_pos, token_type_ids=None,
        ...
        if position_ids is None:
            ...
            position_ids = position_ids.unsqueeze(
                0).expand([batch_size, posids_len])
