...
from utils import _get_attr_from_opt
...
def forward_step(model, inputs, labels, criterion):
    # train model
    tweet_input_ids, gifs, gif_ids = inputs
    if opt.use_gpu:
        tweet_input_ids = tweet_input_ids.cuda()
        gifs = gifs.cuda()
        labels = labels.cuda()
    # with torch.cuda.amp.autocast():
    score = model(tweet_input_ids, gifs)
