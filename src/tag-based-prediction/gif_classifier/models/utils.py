    ...
    if transposed:
        if image_size is None:
            raise NotImplementedError(
                'Unable to dynamically upsample to odd image size.',
            )
        else:
...
class TransposedConv2dStaticSamePadding(nn.ConvTranspose2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, image_size, stride=1, output_padding=0, groups=1, bias=True, dilation=1):
        super().__init__(
            ...
            stride, 0, output_padding, groups, bias, dilation,
        )
        self.stride = self.stride if len(self.stride) == 2 else [
            self.stride[0],
        ] * 2
        ...
        oh, ow = (image_size, image_size) if isinstance(
            image_size, int,
        ) else image_size
        self._oh, self._ow = oh, ow
        sh, sw = self.stride
        # using same calculation in Conv2dStaticSamePadding
        ih, iw = math.ceil(oh / sh), math.ceil(ow / sw)
        self._ih, self._iw = ih, iw
        kh, kw = self.weight.size()[-2:]
        # actual height/width after TransposedConv2d
        actual_oh = (ih - 1) * self.stride[0] + (kh - 1) * \
            self.dilation[0] + self.output_padding + 1
        actual_ow = (iw - 1) * self.stride[1] + (kw - 1) * \
            self.dilation[1] + self.output_padding + 1
        crop_h = actual_oh - oh
        crop_w = actual_ow - ow
    ...
    def forward(self, x):
        ...
        if crop_h > 0 or crop_w > 0:
            x = x[
                :, :, crop_h // 2: -
                (crop_h - crop_h // 2), crop_w // 2: - (crop_w - crop_w // 2)
            ]
